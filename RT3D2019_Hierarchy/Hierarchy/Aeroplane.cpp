...
void Aeroplane::UpdateMatrices(void)
{
	...
	// Parent the propeller to the plan
	...
	mPlaneCameraRot = mRotX * mRotY * mRotZ;
	...
	if (m_bGunCam)
	{
		m_mCamWorldMatrix = mPlaneCameraRot * mTrans * m_mGunWorldMatrix;
	}
	else
	{
		m_mCamWorldMatrix = mPlaneCameraRot * mTrans * test;
	}

	// Get the camera's world position (m_vCamWorldPos) out of m_mCameraWorldMatrix
	XMMatrixDecompose(&temp, &temp, &m_vCamWorldPos, m_mCamWorldMatrix);
	XMMatrixTranspose(m_mCamWorldMatrix);
}
